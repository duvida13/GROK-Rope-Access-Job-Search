...
- Claude `rope-access-job-search`: daily digests + `state/seen_jobs.json` (~2,280 URLs, ~1,043 company strings). Volume comes from repeating queries on Indeed + national boards + specialist boards, then remembering URLs. One day (2026-09-11) still added 28 names nobody would have pre-listed.
- ChatGPT repo: weekday country rotation + a public job index with stable IDs + a source register of portals per country. Closer to a system. Their company list is still only a seed.
...
1. **Query layer** — finds companies we do not know yet. Country × (`rope access` + `IRATA` + local term + blade/LPS titles) × (open web + Indeed country-level + that country’s national portal).
2. **Board layer** — IRATA, Rigg Access, TTR, ANETVA, emploi-cordiste.fr, lalineavertical.com, CareerStructure. Every run.
3. **Employer-memory layer** — grow `context/employers.md` from verified finds. Recheck frequent hirers’ career pages on a cadence. Never invent URLs. The memory file is not a cap.
4. **Dedup / honesty layer** — one campaign = one job. Closed = Company watch. No date = Unverified. Already in another AI repo = only re-list if we confirmed still live.
...
Complete brief = the machine can be rebuilt from this file. Complete ≠ pasting 1,043 company names here.

The rest of this file (candidate, terms, boards, logging, rigour) stays in force.
